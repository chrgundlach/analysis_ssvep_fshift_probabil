...
pl.con1label = F.conds;
pl.con2label = F.conds2;
...
for i_sub = 1:numel(pl.sub2plot)
    for i_RDK = 1:numel(pl.RDKlabel) % loop across positions
        ...
        t.posidx = find(strcmp(pl.elec2plot_cluster_label, pl.label.pos2{i_RDK,i_sub}));
        t.eleci = pl.elec2plot_cluster{t.posidx};
        pl.elec2plot_i=logical(sum(cell2mat(cellfun(@(x) strcmpi({TFA.electrodes.labels},x), t.eleci, 'UniformOutput',false)),1));
        pl.label.elec{i_RDK,i_sub} = vararg2str(pl.elec2plot_cluster{t.posidx});
        
        
        
        
        pl.data_ind(i_RDK,:,:,i_sub) = squeeze(mean(mean(TFA.fftdata_ind(t.fidx2,pl.elec2plot_i,:,pl.time2plot,pl.sub2plot(i_sub)),1),2));
        pl.data_evo(i_RDK,:,:,i_sub) = squeeze(mean(mean(TFA.fftdata_evo(t.fidx2,pl.elec2plot_i,:,pl.time2plot,pl.sub2plot(i_sub)),1),2));
    end
end
...
pl.RDK.pos1 = repmat(pl.label.pos1(:,1),1,prod(size(pl.data_ind,[2 3 4]))); pl.RDK.pos1=pl.RDK.pos1(:);
pl.RDK.pos2 = permute(repmat(pl.label.pos2,[1, 1, prod(size(pl.data_ind,[2 3]))]), [1 3 2]); pl.RDK.pos2=pl.RDK.pos2(:);
pl.RDK.col1 = permute(repmat(pl.label.color_vals,[1, 1, prod(size(pl.data_ind,[2 3]))]), [1 3 2]); pl.RDK.col1=pl.RDK.col1(:);
pl.RDK.col2 = permute(repmat(pl.label.color_label,[1, 1, prod(size(pl.data_ind,[2 3]))]), [1 3 2]); pl.RDK.col2=pl.RDK.col2(:);
...
R_Mat.all = cell2struct([num2cell([pl.RDK.data_ind_l pl.RDK.data_evo_l pl.RDK.data_ind_bc_l pl.RDK.data_evo_bc_l pl.RDK.subs]) ...
    pl.RDK.RDK pl.RDK.pos1 pl.RDK.pos2 pl.RDK.col1 pl.RDK.col2 pl.RDK.freq pl.RDK.elec pl.RDK.con pl.RDK.time]',...
    {'amplitude_induced','amplitude_evoked','modulation_induced','modulation_evoked','subjects',...
    'RDK_id', 'RDK_pos1', 'RDK_pos2', 'RDK_col1','RDK_col2', 'RDK_freq', 'RDK_elec', 'condition', 'time'});
R_Mat.all_t = struct2table(R_Mat.all);
t.path = 'C:\Users\psy05cvd\Dropbox\work\R-statistics\experiments\ssvep_FShiftAlpha\data';
t.datestr = datestr(now,'mm-dd-yyyy_HH-MM');
% t.filename = 'FFT_SSVEP_Amp_data_withoutBehav_sepRDK_peripheral_2elecs';
t.filename = 'FFT_SSVEP_Amp_data_withoutBehav_sepRDK_largeclust_indremoved';
% write to textfile
% writetable(R_Mat.all_t,fullfile(t.path,sprintf('%s_%s.csv',t.filename,t.datestr)),'Delimiter',';')
...
pl.rdk_att_index = [1 2 1 2 1];
pl.pos_idx = [1 1 2 2];
pl.rdk_idx = [1 2 1 2];
pl.lab_time = {'pre';'post'};
pl.lab_con = {'att.';'unatt.'};
pl.lab_pos = {'centr.';'periph.'};
...
pl.data = []; pl.data_ind = [];
for i_sub = 1:numel(pl.sub2plot)
    for i_RDK = 1:numel(pl.pos_idx) % loop across positions
        % extract all relevant information for RDK
        pl.label.pos2{i_RDK,i_sub} = TFA.RDK(pl.sub2plot(i_sub)).RDK(i_RDK).position;
        pl.label.color_vals{i_RDK,i_sub} = vararg2str(TFA.RDK(pl.sub2plot(i_sub)).RDK(i_RDK).col(1,:));
        pl.label.color_label{i_RDK,i_sub} = TFA.RDK(pl.sub2plot(i_sub)).RDK(i_RDK).colname;
        pl.label.freq{i_RDK,i_sub} = TFA.RDK(pl.sub2plot(i_sub)).RDK(i_RDK).freq;
        ...
        % extract data
        pl.data(pl.pos_idx(i_RDK),:,:,i_sub,pl.rdk_idx(i_RDK)) = ...
            squeeze(mean(TFA.data_evo(t.fidx1,:,pl.elec2plot_i,pl.rdk_att_index([0 1]+i_RDK),pl.sub2plot(i_sub)),3));
        pl.data_ind(pl.pos_idx(i_RDK),:,:,i_sub,pl.rdk_idx(i_RDK)) = ...
            squeeze(mean(TFA.data_ind(t.fidx1,:,pl.elec2plot_i,pl.rdk_att_index([0 1]+i_RDK),pl.sub2plot(i_sub)),3));
            
    end
end

% baselince corrected
pl.data_bc = 100.*(bsxfun(@rdivide, pl.data, mean(pl.data(:,pl.base_i(1):pl.base_i(2),:,:,:),2))-1);
pl.data_bc = mean(pl.data_bc,5);
pl.data = mean(pl.data,5);

% baselince corrected (induced data)
pl.data_bc = 100.*(bsxfun(@rdivide, pl.data_ind, mean(pl.data_ind(:,pl.base_i(1):pl.base_i(2),:,:,:),2))-1);
pl.data_bc = mean(pl.data_bc,5);
pl.data = mean(pl.data,5);
% 
% % baselince corrected with average induced data removed
pl.data_bc = 100.*(bsxfun(@rdivide, pl.data, mean(pl.data(:,pl.base_i(1):pl.base_i(2),:,:,:),2))-1);
pl.data_bc_ind = 100.*(bsxfun(@rdivide, pl.data_ind, mean(pl.data_ind(:,pl.base_i(1):pl.base_i(2),:,:,:),2))-1);
pl.data_bc = bsxfun(@minus,pl.data_bc, mean(pl.data_bc_ind,[3 4 5]));
pl.data_bc = mean(pl.data_bc,5);
pl.data = mean(pl.data,5);
